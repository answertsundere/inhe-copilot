...
class AnalysisPipelineService:
    ...
    def _prepare_request(self, request: AnalysisPipelineRequest) -> AnalysisPipelineRequest:
        ...
        if lifecycle_context:
            trusted_eligibility_context["conversation_goal_lifecycle"] = (
                lifecycle_context
            )
        if lifecycle_status != "disabled":
            context.setdefault("pipeline_diagnostics", []).append({
                ...
                "open_goal_count": len(
                    lifecycle_context.get("open_goals") or []
                ),
            })
    ...
    def _record_conversation_goal_lifecycle(
        ...
        """Persist only owner-stamped goals; a candidate reply is not delivery."""
