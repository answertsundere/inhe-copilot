...
class ModelFirstAnswerComposerService:
    ...
    def _system_prompt(cls) -> str:
        ...
        return (
            ...
            "required clause 的 text 只能在所选 allowed_scope 和 required_qualifiers"
            " 允许的范围内回答，保留必要的 restricted boundary；没有被策略授权的帮助不得补造。"
            "allowed_scope 是边界，不是必须列全的场景清单；"
            "只回答当前 goal 和已提供上下文实际相关的部分。"
            "required_qualifiers 中每一项都必须遵守，不等于每一项都要逐条说给客户。"
            "禁止断言某结论的限制，通过不作该断言来遵守；"
            "不得为了声明遵守而引入客户未问的风险话题。"
            "当前所问结论的未知或受限边界必须明确；"
            "要求说明的警示、条件和安全处理动作必须显式表达，不能以简洁为由省略；"
            "语义不明确的 qualifier 不得自行省略。"
