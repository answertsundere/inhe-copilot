...
def _rewrite_fallback_reply(state: dict, fallback_mode: str) -> str:
    ...
    if intent in ("aftersales", "refund", "complaint"):
        if has_order_id:
            return (
                "亲，售后问题我已收到您的订单信息；如果是发错、少件、破损或需要退货/换货，"
                "可以先按平台售后流程处理，我会帮您一起核实处理路径。"
                "\n为了更快确认，方便的话麻烦您把收到的商品、外包装面单或问题位置拍清楚发我；"
                "我这边会结合订单继续跟进。"
            )
        return (
            "亲，售后问题我先帮您记录；如果是发错、少件、破损或需要退货/换货，"
            "可以按平台售后流程处理，我会帮您一起核实。"
            "\n麻烦您补充一下订单号，并把收到的商品或问题位置拍清楚发我，这样处理会更快。"
        )
