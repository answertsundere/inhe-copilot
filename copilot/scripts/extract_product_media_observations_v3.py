...
from app.services.product_media_observation_v3_service import PANEL_BBOX_REPAIR_STAGE, STAGES, ProductMediaObservationV3Extractor, parse_v3_model_json, v3_model_prompt
...
def _summary(rows: list[dict], guard: ReadOnlyDatabaseGuard) -> dict:
    ...
    model_diagnostics = [item for item in diagnostics if item.get("stage") in {*STAGES, PANEL_BBOX_REPAIR_STAGE}]
    proposal_diagnostics = [item for item in diagnostics if item.get("stage") == "panel_proposal"]
    labelled = [item for item in observations if item["observation_type"] == "labelled_measurement"]
    stage_valid = Counter(item["stage"] for item in model_diagnostics if item.get("schema_status") == "valid")
    stage_execution = Counter(item["stage"] for item in model_diagnostics if item.get("execution_status") == "success")
    repair_expected_count = sum(
        1 for row in rows
        if any(item.get("stage") == PANEL_BBOX_REPAIR_STAGE for item in row.get("stage_diagnostics", []))
    )
    ...
    return {
        ...
        "panel_proposal_success_rate": {
            "numerator": sum(1 for item in proposal_diagnostics if item.get("proposal_status") == "ready"),
            "denominator": len(rows),
            "rate": sum(1 for item in proposal_diagnostics if item.get("proposal_status") == "ready") / len(rows) if rows else 0.0,
        },
        "panel_verification_success_rate": {
            "numerator": sum(1 for row in multi_panel_rows if not row.get("rejected_evidence")),
            "denominator": len(multi_panel_rows),
            "rate": sum(1 for row in multi_panel_rows if not row.get("rejected_evidence")) / len(multi_panel_rows) if multi_panel_rows else 0.0,
        },
        ...
        "panel_repair_success_count": sum(1 for item in model_diagnostics if item.get("stage") == PANEL_BBOX_REPAIR_STAGE and item.get("schema_status") == "valid"),
        ...
        "invalid_panel_bbox_count": sum(1 for item in rejected if item.get("reason") == "invalid_panel_bbox"),
        "proposal_drift_exceeded_count": sum(1 for item in diagnostics if item.get("reason") == "proposal_drift_exceeded"),
        "cross_panel_binding_rejected_count": sum(1 for item in rejected if item.get("reason") == "cross_panel_binding_rejected"),
        ...
        "formal_kb_mutation_count": guard.write_attempt_count,
