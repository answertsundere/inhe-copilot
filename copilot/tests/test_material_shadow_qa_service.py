...
def test_real_derived_material_shadow_qa_keeps_supported_composition_and_unresolved_risks():
    ...
    assert result["gold_csr_evaluation"]["passed_count"] == 30
    assert result["gold_csr_evaluation"]["pass_rate"] == 1.0
    assert result["gold_csr_evaluation"]["mutation_suite"]["detected_count"] == 14
    assert result["gold_csr_evaluation"]["requires_human_validation"] is False
