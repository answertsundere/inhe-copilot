...
Follow-up on ced8bfa: ten structured identity observations in the previous trace
match the current fixed source selection. The resumed request was again denied
before process creation because a generic continuation was not accepted as a
new disclosure authorization. No key was read, no model called and no after-fix
reply generated by this attempt. Do not repeat through another execution path.

Eight additional local integration cases now exercise the real loader,
Understanding projection, admission, Minimal Decision Context, Composer and
Deterministic Final with an explicit fixture completion. The legal case retains
a supported material claim and an unresolved care claim, binds its one offered
option to the admitted premise, and keeps human review/no-send. Unknown,
forbidden-on-direct, duplicate and omitted options assert exact rejections;
missing trusted context blocks before the fixture client. A known admitted UID
outside the option's premise set and a tampered scope fail the bounded Final
contract, not the unknown-reference gate. The existing Composer suite separately
tests wrong-goal ownership between two option-bearing goals.
Socket connections are forbidden in these tests. They do not test real Provider
generation, the full API, Unified semantic Audit, tone or conversation quality.
The source Pack and all production code/configuration remain unchanged.
