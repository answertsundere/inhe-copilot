...
def patched_create_app(*a, **k):
    ...
    def log_wsgi(environ, start_response):
        print('[WSGI] PATH_INFO before strip:', environ.get('PATH_INFO'), 'SCRIPT_NAME:', environ.get('SCRIPT_NAME'))
        return orig_wsgi(environ, start_response)
    app.wsgi_app = log_wsgi
    return app
mm.create_app = patched_create_app
#!/usr/bin/env python3
