...
| P1 Gold Conversation Quality | active; reconstructed Fixed-8 and synthetic Fixed-40 Supervisor Assist baselines complete; Autonomous Send provider-blocked | The reconstructed query-only Fixed-8 executes `8/8`; Composer and Deterministic Final pass `8/8`; customer-goal clause coverage is `16/16`; direct attribution is `4/4`; explicit unresolved declaration is `12/12`; partial-answer structural handling is `2/2`; formal-knowledge DML and `can_send` are both `0`; and human review is `8/8`. A later DeepSeek fixed-40 structural run completed Composer and Deterministic Final `40/40`, with no send/write authority, but selected zero evidence, used generic inability-to-confirm language in `32/40`, and exposed one unsupported negative-absence claim. These are safety and structure checkpoints, not demonstrated quality gains: human-quality review is incomplete and `real_customer_accuracy=null` | Freeze and trace the unsupported negative-absence claim through existing owners, then run human review on the frozen replies. Keep Autonomous Send blocked until an independent Audit role, real accuracy, Safety, and Delivery qualify; do not add reply templates or change Delivery based on synthetic rubric results |
