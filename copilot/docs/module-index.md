...
| Candidate reply | `app/services/model_first_answer_composer_service.py`, `app/llm/client.py` | P1.4 engineering checkpoint; Provider role not qualified; opt-in, review-only, disabled | Partitions authoritative current-turn customer goals from dependencies, actions, media, and constraints; performs no Provider call when that partition has zero renderable customer goals; projects each applicable goal's eligible options under deterministic request aliases while retaining canonical policy/Pack identity server-side; accepts only `goal_ref`, customer text, and selected option aliases from the model, restores clause kind and evidence/premise bindings from validated server state, and leaves concrete option choice and wording to the model without adding evidence or send authority. By default it uses the unchanged formal LLM; an explicit complete and qualified COPILOT_COMPOSER_LLM override changes only this role and fails closed when incomplete or unqualified. The Prompt now distinguishes missing evidence from a negative fact, but current MiniMax qualification stopped at `2/3` after factor-budget expansion and an internal-language rejection |
...
| Unified Textual Audit | `app/services/final_semantic_quality_service.py` | formal legacy audit; v3 strict role default-off; engineering contract qualified, model role not qualified | The model-first candidate's sole textual LLM audit for factual faithfulness, unresolved polarity, goal coverage, continuity, query/reply fit, serious repetition, internal-process language, and semantic-budget compliance. It treats a Composer result as a candidate only when that result owns the final reply. V3 receives the complete required-qualifier set and requires independent advice, factor, restricted-boundary, qualifier, and conclusion decisions. Its optional strict path uses one independent role request plus the existing local Validator; configuration, qualification, schema, or Provider failure never falls back to the Composer/formal model. Available local Qwen and MiniMax candidates did not pass the frozen no-test-claim semantic gate, and DeepSeek produced no model result because authentication failed; the role remains disabled and Provider-blocked |
