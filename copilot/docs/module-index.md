...
| Turn understanding | `app/services/semantic_fact_type_service.py`, `app/agent/nodes/query_fact_type_classifier.py`, `scripts/qualify_turn_understanding_strict_provider.py` | formal; strict role default-off | One semantic call returns typed atomic goals with current-message source provenance. Every explicit buyer fact, property, suitability, comparison, guarantee, practical request, or request to determine which service outcome applies remains a `customer_goal`; attributes narrow the requested property rather than identify the product. A request to execute an external side effect now remains a `service_action`; neither kind authorizes the outcome or executes it. Canonical `customer_goals` own goal identity; `requested_claims` are derived and must be rebuilt from them before Claim Resolution, never accepted as a divergent customer-goal source. Canonical dimension goals may additionally declare one closed `subject_scope` (`product`, `packaging`, `component`, `accessory`, or `included_item`); an absent scope never becomes an explicit `product` scope, may select only direct product-overall or legacy-unscoped evidence, never becomes a fact, and cannot replace the requested attribute. Policy candidates are projected only from the trusted Domain Pack with their family, scope, conclusion boundary, qualifiers, prohibited claim families, and optional unmapped semantic boundary. A canonical goal may nominate only an exact family or a type explicitly listed in the trusted policy's canonical_claim_types. An unmapped goal can retain a nominated policy only when its non-authoritative semantic key is explicitly listed by that policy's trusted Pack boundary; a missing or nonmatching key clears the nomination. This is a fail-closed restriction only: it never creates a fact, derives a goal family, selects an option, or authorizes an answer. When a canonical goal omits policy intent, compatible trusted candidates must agree on one goal family before it can be projected; its intent kind is projected only when those candidates agree on one non-empty kind. Multiple families or kinds remain empty. Neither path selects a policy. With an explicit runtime HMAC, the same call may receive an opaque open-goal alias and typed identity metadata, then nominate a same-identity continuation from the current span; historical text, server goal IDs, conversation IDs, and provenance never leave the server. Goals remain ordered by the current-message source span, with server goal identity only as a deterministic tie-breaker. The optional strict role has its own credentials, model, capability, and qualification fingerprint; it never inherits the formal reply role, has no JSON/free-text fallback, and remains disabled until its exact configuration is qualified. Query-fact compatibility fallbacks stay degraded, while supporting dependencies, service actions, and contextual constraints remain distinct |
...
| Role-scoped strict model transport | `app/services/strict_decision_provider_service.py`, `app/config.py`, `scripts/qualify_unified_audit_role.py`, `scripts/qualify_turn_understanding_strict_provider.py` | formal utility; role-specific default-off qualification | One native strict-schema or strict-tool request for an explicitly configured role; Unified Audit and Turn Understanding credentials/model/capability never inherit the formal Agent or decision-shadow role, and missing/unqualified configuration fails before a model call with no fallback. Provider-specific transport projection may remove only documented incompatible schema keywords while the local source schema remains the Validator. Read-only role qualifiers use synthetic data, local production validators, safe provider metadata, and a configuration fingerprint; they never change qualification state, feature flags, evidence, replies, or send authority |
| Product/order identity | identity services and resolver nodes | formal | External references resolve to JST/internal scoped identity. Structured sidebar order values preserve adapter type; an untyped value is a bounded `unknown_identifier` lookup rather than a length-based platform-ID guess. An explicit shop ID, or a shop label resolving to exactly one enabled JST shop, is channel-routing context only: paired with an explicit order reference, it can constrain a direct outbound lookup and bounded scan, but is never identity or evidence itself. An unmapped, ambiguous, or unavailable display label cannot enable a row-list scan, yet it cannot suppress exact `o_id`/`so_id` or direct outbound identifier queries; a direct miss returns the scope reason. Every JST response must contain one exact order-level or item-level identity-field match; a nonempty list, first row, substring, title, or cross-namespace `i_id` is not proof. Standard OpenAPI Tmall misses remain unresolved; ADR 0011 additionally permits a default-disabled, manifest-verified, identity-only local snapshot lookup for one unique exact `outer_oi_id`. It can seed only exact SKU -> Product Hub fact reads, never historical order/logistics state or delivery. A verified single live order item, one primary item with gifts, or one integration-verified exact outbound item is projected through the existing selection policy as internal product name/SKU/`i_id`; ambiguity remains unresolved and cannot scope facts. |
...
| Product context candidates | `app/services/product_context_pack_service.py`, `app/integrations/product_hub/reviewed_facts_client.py` | formal; Product Hub facts and review-only media default-off | Preserve role, review, identity, attribute, and provenance metadata. For a JST-derived identity, the optional Hub reader verifies an exact SKU through the Hub SKU endpoint before using its returned Hub product code for facts. A single-item, single-primary-item-with-gifts, integration-verified exact outbound-item, or manifest-verified identity-only snapshot item may supply that exact SKU directly; a context-selected multi-item order or conflicting SKU cannot bypass normal identity resolution. Snapshot identity is not order/logistics state. JST `i_id` never crosses namespaces as a Hub product code. A product-level Hub fact may use that exact SKU only as its effective admission scope while retaining its original Hub scope in provenance. A separate exact-SKU asset projection accepts only `approved`/`live` known image types and canonical preview routes as `media_reference`, `reference_only`, manual-review candidates; it never becomes formal evidence, `recommended_assets`, a reply block, or delivery authority. Neither path is a title search, catalog snapshot, semantic asset search, or independent evidence registry |
...
| Claim resolution | `app/services/claim_resolution_service.py` | formal utility; bounded inference disabled with Composer path | Preserves supported, unresolved, conflicting, and prohibited direct-evidence state; for a scoped dimension goal, direct evidence must match both canonical attribute and canonical object scope. Aggregate `overall_dimensions` is a separate attribute and is never decomposed into width, height, depth, or any other axis. For an unscoped dimensions goal, an exact product-scoped `overall_dimensions` record may be selected only when every other admitted candidate is unattributed legacy evidence; an explicit axis, unscoped structured record, or conflict still remains ambiguous or blocked, while non-product measurements remain outside this product selection. It keeps restricted request risk separate from low/medium answer-strategy risk and may project a practical alternative only from the deterministic trusted Domain Pack/admitted-premise/scope/conflict intersection for the same owner-stamped goal family. Missing goal-family authority yields no option; an already-supported goal remains direct-only unless an exact trusted practical intent was nominated. Claim Resolution never selects an option |
| Domain policy data | `app/repositories/file_policy_repository.py`, `rules/domain_policy_packs/`, reviewed `KBProduct.domain_policy_id` | formal data utility; no routing authority | Sole strict loader for versioned claim policy; resolves trusted deployment, a Pipeline-verified mapping from one exact SKU/`i_id` to one published product, or an isolated evaluation selector, emits an anonymous Pack reference/content hash projection, and revalidates both before use. In the opt-in `product_hub_review_only` source, the existing identity resolver/client instead verify active exact SKU/product/passport membership and project only `domainPolicyId`; no local/global fallback, source notes, source prompts or evidence promotion. This Hub control binding is not human approval. Outside that source, changing the local published-product mapping still returns it to review. Query-only legacy catalogs lacking the column stay readable and unbound. Policies contain premise families, scope, risk ceilings, bounded advice modes, qualifiers, prohibitions and mandatory review, but no product facts, customer text or reply templates |
...
The default-disabled v2 identity-only JST snapshot may additionally accept an
adapter-typed internal order reference only by an exact HMAC match and only when
it resolves one eligible item. It has the same narrow SKU-to-Product-Hub scope
as the existing exact snapshot-item path; it cannot expose or derive order,
logistics, refund, media-delivery, or send state.

The default-off Product Hub fact bridge has one additional scope-preserving
aggregate: an authoritative `packaging` dimension goal may consume only a
complete, confirmed, exact-SKU set of carton length, width, and height fields.
It remains packaging-scoped with per-field provenance, and cannot answer an
unscoped or product-overall dimension goal.
...
| QianNiu/PDD/JD adapters | canonical adapter ports; existing `scripts/qianniu_sidecar.py`, `scripts/sidecar/uia_sidebar_extractor.py`, `scripts/sidecar/context_parser.py`, Sidecar route and real-test template | native selection blocked; explicit assisted document import development-only; continuous capture unqualified | Default-off direct-loopback human/CSRF preview. Assisted mode requires buyer/shop re-entry and acknowledgment; all turns remain historical, current question blank, fresh local conversation. UIA/MSAA conflicts block; no native identity/Gold authority, shared cache, polling/model/send or unbound order/product import |
