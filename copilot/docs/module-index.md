...
| Product/order identity | identity services and resolver nodes | formal | External references resolve to JST/internal scoped identity. A sidebar order identifier is resolved before downstream answering. Public `shop_id` is the platform-neutral logical store reference; only a separately configured `jst_shop_id` may filter a JST provider request, so future integrated stores cannot collide or accidentally filter out a valid order. The Tool Registry propagates that provider identity into the same dispatcher used by the legacy node. Exact outbound-order lookup does not add a recent modification-time window: the supplied order identifier and optional provider shop identity are validated against the returned row instead. The fallback external-order scan follows every provider page to an exact order- or item-level identifier match or the real last page; repeated full pages fail closed as pagination stalls, and the explicit JST shop scope survives the entire scan. A completed exhaustive miss is a completed tool result and cannot trigger the same legacy lookup a second time; infrastructure errors and bounded fast-path misses retain the existing fail-closed fallback. Logistics tool results reuse the same item-selection contract to hydrate order status plus internal product name, `sku_id`, and `i_id`. A completed live logistics lookup remains the reply-strategy authority for that turn and cannot be displaced by a generic ETA-certainty concern. A single reliable item may resolve, while multi-item ambiguity remains explicit. Full logistics identifiers stay in structured internal context and customer-visible replies use a masked reference. The read-only JST transport retries official rate-limit codes `199/200` once after a bounded delay; other API errors and a repeated rate-limit response still fail closed. Generic JST APIs cannot expose Taobao/Tmall online order identifiers; those mappings must come from the QianNiu Sidecar or a separately authorized Qimen adapter and must not be misreported as nonexistent orders. Evidence-enabled evaluation uses the same exact structured identity contract: a local reviewed Sidecar may supply `sku`/`i_id`/product or order identity, while titles, conversation text, historical replies, and semantic similarity have no authority. Missing or ambiguous mappings are input/context gaps and fail before evidence-coverage conclusions |
| Operational fact admission | `app/agent/nodes/evidence_builder.py`, `app/services/admitted_answer_context_service.py` | formal when Formal Evidence Convergence is enabled; review-only | Completed read-only order/logistics tool results may enter canonical evidence only as `operational_fact_direct`, with explicit completed execution status, closed operational scope, compatible claim type, and direct-answer permission. They remain distinct from reviewed product facts and never write product knowledge. Planned/failed tools, write-capable operations, service actions, media candidates, placeholders, and incompatible facts fail closed. This role can support an order/logistics clause but cannot change `can_send`, delivery, or human-review requirements |
