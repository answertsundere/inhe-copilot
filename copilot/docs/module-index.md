...
| P1 Gold conversation baseline | `scripts/run_p1_gold_conversation_baseline.py`, `app/services/high_quality_long_conversation_review_service.py` | 仅限开发诊断。旧 real-derived 合同保持兼容；恢复后新增独立 `conversation-reconstructed-v1` 合同，固定 8 条/40 个历史回合，并明确不是原 Fixed-8 或真实 Gold。统一 Owner 校验目标 provenance、控制引用别名、隐私、版本化哈希、query-only 知识快照和正式知识 DML；评测字段不得进入 Agent payload，重建模式强制 `can_send=false`、`requires_human_review=true`、`real_customer_accuracy=null`。当前只完成确定性预检，尚无本源码 5013 原生八案例结果 |
