...
| LLM decision proposal | `app/services/agent_decision_proposal_service.py`, `app/services/strict_decision_provider_service.py`, `scripts/qualify_strict_decision_provider.py` | shadow-only / qualification-gated | Uses a separately configured, strict-schema provider after the formal graph. It stays disabled unless the exact provider passes the read-only qualification suite, including truncation rejection, and is explicitly locally accepted; application-owned read-only lookups remain bounded, order/media actions stay deferred, and formal decisions stay immutable |
