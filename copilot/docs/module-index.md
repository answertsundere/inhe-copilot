...
| Candidate reply | `app/services/model_first_answer_composer_service.py` | P0-correctness-qualified; P1.2c bounded inference live gate pending; opt-in, review-only, disabled | Partitions authoritative current-turn customer goals from supporting dependencies, service actions, media context, and contextual constraints; for each customer goal selects zero or one goal-scoped eligible policy and must copy its policy, admitted premises, and exact scope into an `allowed_inference` clause; rejects wrong-goal/unknown/duplicate refs and never owns tools, facts, delivery, or send permission |
...
| Bounded low-risk inference | P1.2c option-funnel candidate; default-off, representative live gate pending | Domain Pack supplies safe policies, Claim Resolution filters options, Composer selects zero/one per goal, Final validates, and Unified Audit checks text; no production authority |
