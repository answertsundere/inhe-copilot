...
| Admitted answer context | `app/services/admitted_answer_context_service.py`, `scripts/trace_evidence_convergence.py` | formal utility / read-only | Owns identity-, review-, role-, claim-, placeholder-, and conflict-aware evidence admission; separates direct product facts, policy facts, handoff actions, media candidates, rejected evidence, and unresolved claims. It emits a shadow-only convergence trace from Product Context Pack through formal selection and decision-model context |
