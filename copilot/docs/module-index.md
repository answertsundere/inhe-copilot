...
| Product/order identity | identity services and resolver nodes | formal | External references resolve to JST/internal scoped identity. A sidebar order identifier is resolved before downstream answering. Public `shop_id` is the platform-neutral logical store reference; only a separately configured `jst_shop_id` may filter a JST provider request, so future integrated stores cannot collide or accidentally filter out a valid order. Exact outbound-order lookup does not add a recent modification-time window: the supplied order identifier and optional provider shop identity are validated against the returned row instead. Logistics tool results reuse the same item-selection contract to hydrate order status plus internal product name, `sku_id`, and `i_id`. A completed live logistics lookup remains the reply-strategy authority for that turn and cannot be displaced by a generic ETA-certainty concern. A single reliable item may resolve, while multi-item ambiguity remains explicit. Full logistics identifiers stay in structured internal context and customer-visible replies use a masked reference. The read-only JST transport retries official rate-limit codes `199/200` once after a bounded delay; other API errors and a repeated rate-limit response still fail closed. Evidence-enabled evaluation uses the same exact structured identity contract: a local reviewed Sidecar may supply `sku`/`i_id`/product or order identity, while titles, conversation text, historical replies, and semantic similarity have no authority. Missing or ambiguous mappings are input/context gaps and fail before evidence-coverage conclusions |
