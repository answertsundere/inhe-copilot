...
| LLM decision proposal | `app/services/agent_decision_proposal_service.py` | shadow-only | Uses strict intake and proposal schemas after the formal graph; the application may run registered local read-only lookups before admission, while order/media actions remain deferred and formal decisions stay immutable |
