...
| Product media observations | `app/services/product_media_observation_service.py`, `scripts/extract_product_media_observations.py`, `scripts/diagnose_product_media_vlm_provider.py`, `scripts/qualify_product_media_vlm.py` | shadow-only | Offline pending candidates with actual-byte SHA-256, exact type/key and region contracts, low-risk allowlist, high-risk deny registry, ambiguous-variant dimension rejection, bounded transport and provider qualification diagnostics, and SQLite query-only reads; never read by the formal Product Evidence Pack, Grounded Reasoning, or delivery path |
