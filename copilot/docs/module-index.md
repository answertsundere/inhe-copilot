...
| Grounded Reasoning Draft | `app/services/grounded_reasoning_draft_service.py`, `scripts/*grounded_reasoning_positive_eval*` | shadow/blocked from promotion | Uses direct, reviewed, identity-scoped evidence only; the read-only positive eval measures admission, draft relevance, and safety boundaries without affecting formal decisions |
