...
| Admitted answer context | `app/services/admitted_answer_context_service.py`, `scripts/trace_evidence_convergence.py` | formal utility / opt-in convergence | Owns identity-, review-, role-, claim-, placeholder-, and conflict-aware evidence admission; emits deterministic canonical selected evidence only from admitted direct facts when `COPILOT_FORMAL_EVIDENCE_CONVERGENCE_ENABLED` is enabled, while actions and media remain non-factual context roles. |
| LLM decision proposal | `app/services/agent_decision_proposal_service.py`, `app/services/claim_resolution_service.py`, `app/services/strict_decision_provider_service.py`, `scripts/qualify_strict_decision_provider.py`, `scripts/run_supervisor_partial_answer_preview_eval.py` | shadow-only / qualification-gated | Claim Resolution assigns stable identities and keeps supported, unresolved, conflicting, and explicitly prohibited claims independent. The provider-independent preview deterministically renders only admitted supported clauses plus pending/conflict review wording; strict-provider polish remains blocked until qualification. Neither path changes formal decisions. |
