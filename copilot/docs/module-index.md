...
| Claim resolution | `app/services/claim_resolution_service.py` | formal utility; bounded inference disabled with Composer path | Preserves supported, unresolved, conflicting, and prohibited direct-evidence state; projects the deterministic trusted Domain Pack/admitted-premise/risk/conflict intersection as goal-scoped `eligible_policy_options`, while an optional trusted intent may narrow but never select the set |
...
| Candidate reply | `app/services/model_first_answer_composer_service.py` | P0-correctness-qualified; P1.2d bounded inference not qualified; opt-in, review-only, disabled | Partitions authoritative current-turn customer goals from dependencies, actions, media, and constraints; may select zero or one goal-scoped eligible policy only after validating the Pipeline-owned Pack reference/hash and admitted premises. It copies policy, premises, and scope into an `allowed_inference` clause without evidence or send authority |
...
| Bounded low-risk inference | P1.2d trusted-context candidate; default-off, not qualified | Pipeline-owned Domain Policy context now propagates separately from evidence and is hash-checked by Claim Resolution, Composer, and Final. Missing/invalid/stale/public-injected context produces no options; live 1/4/16 qualification is still required and no production authority is granted |
