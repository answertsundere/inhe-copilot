...
| Product/order identity | identity services, JST integration adapters and resolver nodes | formal | External references resolve to JST/internal scoped identity. A sidebar online order identifier is resolved before downstream answering. Public `shop_id` is the platform-neutral logical store reference, a separately configured `jst_shop_id` scopes provider records, and controlled `shop_platform` metadata selects the provider capability contract without parsing shop names. Taobao/Tmall customer-service lookup uses only the ordinary read-only JST sales-outbound endpoint `orders/out/simple/query` with exact `so_ids`; unsupported ordinary-order fallback scans are skipped. It requires neither QianNiu Sidecar nor Qimen and projects only outbound/logistics state plus item internal name, `sku_id` and `i_id`. Buyer and receiver data are discarded. A zero-row outbound result remains unresolved and is not proof that the platform order does not exist. Product Hub then resolves the returned exact identity as reference-only, while reviewed formal knowledge remains product-fact authority. Multi-item ambiguity, provider errors and identity conflicts fail closed. Full logistics identifiers stay in structured internal context and are excluded from model-visible fact prose; customer-visible logistics clauses do not need to repeat them. A completed live logistics lookup remains the reply-strategy authority for that turn and cannot be displaced by a generic ETA-certainty concern. The JST transport retries official rate-limit codes `199/200` once after a bounded delay. Titles, conversation text, historical replies and semantic similarity have no identity authority. Missing or ambiguous mappings remain input/context gaps and fail before evidence-coverage conclusions |
...
| Product Data Hub exact fact and media projection | `app/integrations/product_data_hub/read_client.py`, `app/services/product_context_pack_service.py`, existing evidence/admission services | Optional, default-off, read-only catalog adapter. Only a single exact active parent/SKU resolution can project confirmed, non-conflicting identity-aligned facts into the existing `product_facts` channel; the existing Evidence Filter, Builder, and Admitted Answer Context remain the formal admission owners. `product_data_hub_confirmed` is a material-field provenance token produced only at this exact boundary. A directly admitted Hub attribute label remains bound to its value for deterministic customer rendering and grounding, but cannot bypass the existing identity, scope, review, or high-risk gate. A Hub `parts` list is an `included_items` fact only for the exact selected SKU; it cannot answer whether an accessory is separately purchasable. Product-wide `color_options` may aggregate only non-conflicting confirmed color facts for active sibling SKUs under that same exact product identity; it never imports a sibling SKU's specification, size, included-items, or other facts. Hub labels select only role-compatible media candidates and never become facts. High-risk claims stay blocked; product/packaging/component/accessory/included-item scope remains explicit. The Hub customer-service keyword endpoint is excluded. No write, model, reply-owner, sender, or `can_send` authority is granted. |
