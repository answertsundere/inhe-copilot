...
| Product media observations | `app/services/product_media_observation_service.py`, `app/services/product_media_observation_v3_service.py`, `app/services/product_media_observation_review_service.py` | shadow-only / v2 legacy staging / v3 planned review | V3 stages image classification, object localisation, label localisation, and object-label binding for a product, packaging, component, accessory, included item, or display prop. It normalises regions and records a shadow Product Understanding Graph only; it remains outside Product Evidence Pack, Grounded Reasoning, and delivery until a later review and evaluation gate passes. |
