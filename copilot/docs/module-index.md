...
| Product/order identity | identity services and resolver nodes | formal | External references resolve to JST/internal scoped identity. Evidence-enabled evaluation uses the same exact structured identity contract: a local reviewed Sidecar may supply `sku`/`i_id`/product or order identity, while titles, conversation text, historical replies, and semantic similarity have no authority. Missing or ambiguous mappings are input/context gaps and fail before evidence-coverage conclusions |
