...
| Product Data Hub identity reference | `app/integrations/product_data_hub/read_client.py`, `app/services/product_context_pack_service.py` | Optional, default-off, read-only catalog adapter. It matches active product/SKU codes exactly, rejects ambiguous or parent-conflicting identities, and exposes only identity/display metadata as `catalog_reference`. Hub notes, SKU free-form attributes, media labels, and `/api/v2/cs/ask` are excluded. The projection is always `reference_only`, cannot populate facts or selected evidence, and grants no model, reply, action, or send authority. The Hub customer-service tables are currently empty and its keyword-answer path is not an Agent integration surface. |
