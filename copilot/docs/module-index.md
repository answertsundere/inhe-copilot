...
| Product media observations | `app/services/product_media_observation_service.py`, `scripts/extract_product_media_observations.py` | shadow-only | Offline pending candidates with product scope, content-hash provenance, confidence, and optional regions; never read by the formal Product Evidence Pack, Grounded Reasoning, or delivery path |
