...
| Claim resolution | `app/services/claim_resolution_service.py` | formal utility; bounded inference disabled with Composer path | Supported, unresolved, conflicting, and prohibited state from admitted context; when a trusted policy is explicitly nominated, it alone binds admitted premises to scope, risk ceiling, review-only status, and policy provenance |
...
| Candidate reply | `app/services/model_first_answer_composer_service.py` | P0-correctness-qualified; bounded inference live gate not qualified; opt-in, review-only, disabled | Partitions authoritative current-turn customer goals from supporting dependencies, service actions, media context, and contextual constraints; renders an `allowed_inference` clause only from Claim Resolution policy/premise/scope/risk metadata, and otherwise rejects non-customer clauses, invalid provenance, duplicate/unknown refs, and missing goals; no tools, fact creation, delivery, or send permission |
...
| Bounded low-risk inference | contract checkpoint; representative live gate `0/4`, disabled | Existing Domain Pack, Claim Resolution, Composer, Final, and Unified Audit owners only; no production authority |
