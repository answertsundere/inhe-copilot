...
| Product/order identity | identity services and resolver nodes | formal | External references resolve to JST/internal scoped identity. A sidebar order identifier is resolved before downstream answering; logistics tool results reuse the same item-selection contract to hydrate order status plus internal product name, `sku_id`, and `i_id`. A single reliable item may resolve, while multi-item ambiguity remains explicit. Full logistics identifiers stay in structured internal context and customer-visible replies use a masked reference. Evidence-enabled evaluation uses the same exact structured identity contract: a local reviewed Sidecar may supply `sku`/`i_id`/product or order identity, while titles, conversation text, historical replies, and semantic similarity have no authority. Missing or ambiguous mappings are input/context gaps and fail before evidence-coverage conclusions |
