...
| Product/order identity | identity services, JST integration adapters and resolver nodes | formal | External references resolve to JST/internal scoped identity. A sidebar order identifier is resolved before downstream answering. Public `shop_id` is the platform-neutral logical store reference; a separately configured `jst_shop_id` scopes provider records and deployment-owned `order_lookup_provider` selects the read-only transport. Standard stores retain the ordinary JST exact/paginated lookup. Taobao/Tmall stores use the separately authorized Qimen `jushuitan.order.list.query` transport because generic JST APIs exclude those online order identifiers. Qimen requires an exact `so_id` result and projects only order/logistics state plus item internal name, `sku_id` and `i_id`; buyer and receiver data are discarded. Missing Qimen configuration fails as `provider_not_configured` without a generic fallback or a false order-not-found claim. Product Hub then resolves the returned exact identity as reference-only, while reviewed formal knowledge remains product-fact authority. Multi-item ambiguity, provider errors and identity conflicts fail closed. Full logistics identifiers stay in structured internal context and customer-visible replies use a masked reference. A completed live logistics lookup remains the reply-strategy authority for that turn and cannot be displaced by a generic ETA-certainty concern. The standard JST transport still retries official rate-limit codes `199/200` once after a bounded delay. Titles, conversation text, historical replies and semantic similarity have no identity authority. Missing or ambiguous mappings remain input/context gaps and fail before evidence-coverage conclusions |
