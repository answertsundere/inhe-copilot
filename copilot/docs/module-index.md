...
| Product context candidates | `app/services/product_context_pack_service.py`, `app/integrations/product_hub/reviewed_facts_client.py` | formal; Product Hub reader default-off | Preserve role, review, identity, attribute, and provenance metadata. The optional Hub reader is one bounded read-only candidate source after exact resolver identity, never a title search, catalog snapshot, media source, or independent evidence registry |
