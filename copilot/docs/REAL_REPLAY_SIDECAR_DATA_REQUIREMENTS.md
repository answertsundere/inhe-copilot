...
侧栏商品标题不是最终商品事实，只是商品身份线索。Replay 和在线 Agent 应先保留侧栏标题原文中的类目、场景词，再进入身份解析链路：

```text
千牛侧栏商品标题 -> 本地商品名匹配 / 聚水潭商品名查询 -> 内部 SKU 或 i_id -> KBProduct -> 商品字段、素材、政策证据
```

商品标题里常见的“客厅、卧室、桌面、儿童”等词不能按地址 PII 直接脱敏，否则会破坏商品身份解析。标题字段仍需要脱敏 URL、密钥、手机号、账号、长 ID 等敏感信息。
