...
Full-app authentication/canonical-entry regression and a no-credential HTTP
check are now recorded in [runtime operation](runtime-operation.md#isolated-manual-qianniu-workbench).
Page authorization is stronger than preview-API authorization; HTTP 200 alone
does not prove a nonempty, eligible candidate. The adapter is not deployed to
the existing production ports by this checkpoint.
